...
func (s *httpServer) rebuildBeaconIDHeaders() {
	// Always include both: the listener's default header AND X-Beacon-Id
	// as the pre-profile bootstrap fallback for new agent registration.
	seen := map[string]bool{s.hbHeader: true, "X-Beacon-Id": true}
	headers := []string{s.hbHeader}
	if s.hbHeader != "X-Beacon-Id" {
		headers = append(headers, "X-Beacon-Id")
	}
