...
func parseProfileConfig(cfg map[string]any) *ProfileConfig {
	...
	// v2 JSON from profile_json textarea (user pastes JSON + clicks Parse)
	...
	// v2: profile_json_file — user selected a .json file via selector_file widget;
	// server reads and parses it (ax.file_read is not available in AxScript v2).
	if filePath, ok := cfg["profile_json_file"].(string); ok && filePath != "" {
		naxListenerLogInfo("parseProfileConfig: reading profile_json_file %s", filePath)
		if data, err := os.ReadFile(filePath); err == nil {
			...
			naxListenerLogErr("parseProfileConfig: failed to read %s: %v", filePath, err)
		}
	}
