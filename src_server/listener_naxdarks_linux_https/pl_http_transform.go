...
func extractMeta(cfg *OutputConfig, r *http.Request) (string, error) {
	...
	case "header":
		// Use Values() to get all header values. When beacon_id_header and
		// metadata use the same header name (e.g. "Authorization"), the
		// beacon ID is the first value and metadata is the last.
		vals := r.Header.Values(cfg.Name)
		if len(vals) == 0 {
