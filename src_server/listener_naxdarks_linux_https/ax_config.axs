...
function ListenerUI(mode_create)
{
    ...
    function listToArray(w) {
        let a = [];
        try { for (let i = 0; i < w.count(); i++) a.push(w.text(i)); } catch(e) {}
        return a;
    }
    function headersToObj(w) {
        let o = {};
        try { for (let i = 0; i < w.count(); i++) { let v = w.text(i); let x = v.indexOf(": "); if (x > 0) o[v.substring(0,x)] = v.substring(x+2); } } catch(e) {}
        return o;
    }
    function paramsToObj(w) {
        let o = {};
        try { for (let i = 0; i < w.count(); i++) { let v = w.text(i); let x = v.indexOf("="); if (x > 0) o[v.substring(0,x)] = v.substring(x+1); } } catch(e) {}
        return o;
    }
    ...
    // ========= [ SSL — mandatory for this listener ] =========
    ...
    netLayout.addWidget(form.create_label("SSL Certificate:"), 2, 0);
    netLayout.addWidget(certSelector, 2, 1, 1, 2);
    netLayout.addWidget(form.create_label("SSL Private Key:"), 3, 0);
    netLayout.addWidget(keySelector,  3, 1, 1, 2);
    netLayout.addWidget(form.create_vspacer(), 4, 0, 1, 3); // absorb remaining space
    ...
    let btnExport     = form.create_button("Build JSON");
    ...
    function b64decode(b64) {
        ...
        for(var i=0;i<b64.length;){
            var e1=C.indexOf(b64[i++]),e2=C.indexOf(b64[i++]),e3=C.indexOf(b64[i++]),e4=C.indexOf(b64[i++]);
    ...
    // v2: ax.file_read gone → ax.encode_file("base64", path) + pure-JS b64decode
    form.connect(btnImport, "clicked", function() {
        let path = ax.prompt_open_file("Import Profile JSON", "*.json");
        if (path) {
            try {
                let b64 = ax.encode_file("base64", path);
                if (b64) {
                    let json = b64decode(b64);
